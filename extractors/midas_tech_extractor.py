    ...
    def delete_existing_records(df):
        with ENGINE.begin() as conn:
            for _, row in df.iterrows():
                query = text(
                    f"DELETE FROM {MIDAS_TECH_TABLE} WHERE wenco_id = :wenco_id AND date = :date AND last_name = :last_name")
                result = conn.execute(query,
                                      {'wenco_id': row['wenco_id'], 'date': row['date'], 'last_name': row['last_name']})
                print(
                    f"Query executed: DELETE FROM {MIDAS_TECH_TABLE} WHERE wenco_id = :wenco_id AND date = :date AND last_name = :last_name with params wenco_id={row['wenco_id']}, date={row['date']}, last_name={row['last_name']}")
